...
const getProductById = async (req: Request, res: Response) => {
    try{
        ...
        if(!product) return res.status(404).send(`product with id ${id} not found`);
        return res.send({
            ok: true,
            product
        });
...
const getProductbyUserId = (req: Request, res: Response) => {
...
const createProduct = async(req: Request, res: Response) => {
    ...
    try{
        let product = new Product(body);
         if(!product) return res.status(400).send("Error creating product")
...
const updateProduct = async(req: Request, res: Response) => {
    try{
        ...
        if(!product) return res.status(400).send("Error updating product");
